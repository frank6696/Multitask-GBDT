...
<<<<<<< HEAD
  ...
  // single training
  vector<vector<float>> single_loss_record;
=======
...
>>>>>>> 0316b0c0c275f2dea42066b9cb8634cdb0a877c3
...
<<<<<<< HEAD
        ...
        // calculate new gradient.
        loss->get_gradient(preds[j], datasets[j].get_label_data(), gradient);
        datasets[j].set_gradients(gradient);
      }

      if (i == single_num_round) break;
      Tree *new_tree =
          new Tree(max_depth, lambda, beta, feature_size, min_sample_leaf, learning_rate, loss, single_objective);
//          cout<<"this is the "<<j<<"th task"<<endl;
      new_tree->train(datasets[j]);
      single_trees[j].push_back(new_tree);
    }
    if (i != 0) {
      single_loss_record.push_back(tmp_loss);
    }
  }

  if (early_stop) {
    // get best iteration for each task
    for (int i = 0; i < dataset.get_task_num(); ++i) {
      for (int j = 0; j < this->common_num_round; ++j) {
        if (j == 0) {
          this->commmon_best_iterations.push_back(make_pair(1, common_loss_record[j][i]));
=======
        float loss_score = 0.0f;
